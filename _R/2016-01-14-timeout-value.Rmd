...
df <- read_csv("../files_R/data/pbp_cleaned.csv")
df_plays <- df %>%
  filter(type != "CONV")

# Custom features
df_plays %<>%
  mutate(pts = ifelse(is.na(pts), 0, pts),   # Replace pts with 0 if missing
         # Determine which half the play occurs in
         half_two = as.numeric(qtr >= 3))

# Features to use in the model
features <- c("dwn", "ytg", "yfog", "secs_left",
              "score_diff", "timo", "timd", "spread",
              "kneel_down", "qtr", "half_two")
target <- c("win", "pts")

# Minimize df_plays to relevant variables and remove missing data
df_plays %<>%
...
Here I find an effect on par with Burke's original estimate. The untransformed coefficient for `r coef(wp_logit)[6]` for defensive timeouts, close to Burke's -0.111. However the coefficient for offensive timeouts is even stranger at `r coef(wp_logit)[5]`. This suggests that as an offense's remaining timeouts increase, its win probability *decreases*.

```{r wp_logit_off}
data_frame(score_diff = 3,
           yfog = 50,
           secs_left = rep(1800:900, times = 4),
           timd = 0,
  ...
  scale_x_reverse() +
  scale_y_continuous(labels = scales::percent) +
  labs(title = "Offense Win Probability by Time and Offensive Timeouts Left",
       x = "Time Left",
       y = "Win Probability",
       color = "Offensive\nTimeouts Left") +
  theme_bw() +
...
data_frame(score_diff = 3,
  ...
  mutate(wp = predict(wp_logit, newdata = ., type = "response"),
         wp_se = predict(wp_logit, newdata = ., type = "response", se.fit = TRUE)$se.fit) %>%
  ggplot(aes(secs_left, wp, color = factor(timo), fill = factor(timo),
             ymin = wp - 1.96 * wp_se,
             ymax = wp + 1.96 * wp_se)) +
  geom_line(size = 1) +
  geom_ribbon(alpha = .2) +
