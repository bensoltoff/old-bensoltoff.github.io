...
For a standard 12 game match, the probability the better player wins is `r with(varying_matches, prob_win[n_matches == 12])`. To achieve a 75% or better probability of success, the match length should be `r which(abs(varying_matches$prob_win - .75) == min(abs(varying_matches$prob_win - .75)))`. For a 90% chance, it should be a best of `r which(abs(varying_matches$prob_win - .90) == min(abs(varying_matches$prob_win - .90)))` games match. For 99%, make that best of `r which(abs(varying_matches$prob_win - .99) == min(abs(varying_matches$prob_win - .99)))`.
